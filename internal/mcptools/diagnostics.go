...
func RegisterDiagnostics(s *server.MCPServer, reg *registry.Registry, pool *agentclient.Pool, auditLog *audit.Log, leaseCfg LeaseGateConfig) {
	addTool(s,
		mcp.NewTool("collect_etw_trace",
			mcp.WithDescription("Collect an ETW/WPR trace on a remote node and pull the .etl locally."),
			mcp.WithString("node", mcp.Required(), mcp.Description("Node name from registry")),
			mcp.WithString("profile", mcp.Required(), mcp.Description("WPR profile name or path to .wprp file on node")),
	...
	addTool(s,
		mcp.NewTool("get_crash_dumps",
			mcp.WithDescription("List and optionally pull crash dumps from a remote node."),
			mcp.WithString("node", mcp.Required(), mcp.Description("Node name from registry")),
			mcp.WithString("local_dir", mcp.Description("Pull all dumps to this local directory")),
		),
	...
	addTool(s,
		mcp.NewTool("sync_time",
			mcp.WithDescription("Force w32tm resync on all nodes or a specific topology."),
			mcp.WithString("topology", mcp.Description("Topology to sync; omit for all nodes")),
		),
	...
	addTool(s,
		mcp.NewTool("disable_kd",
			mcp.WithDescription("Disable kernel debugging on a remote node."),
			mcp.WithString("node", mcp.Required(), mcp.Description("Node name from registry")),
	...
	addTool(s,
		mcp.NewTool("get_kd_status",
			mcp.WithDescription("Query kernel debugging settings on a remote node."),
			mcp.WithString("node", mcp.Required(), mcp.Description("Node name from registry")),
		),
