...
func RegisterLeasing(s *server.MCPServer, reg *registry.Registry, store leasing.Store) {
	addTool(s,
		mcp.NewTool("lease",
			mcp.WithDescription("Acquire an exclusive lease over one or more lab nodes so other LabLink sessions don't disturb them. Fail-fast by default; pass wait_seconds>0 to block until the lease becomes available. Default duration is 60 minutes; bump with extend_lease. Release with release(lease_id=...) when done."),
			mcp.WithArray("nodes", mcp.Required(), mcp.Description("Node names to lease (atomic — all-or-nothing). Must all exist in the registry."), mcp.WithStringItems()),
			mcp.WithNumber("duration_minutes", mcp.Description("Lease TTL in minutes. Default 60.")),
			mcp.WithNumber("wait_seconds", mcp.Description("If >0, block up to this many seconds waiting for contested nodes to become free. Default 0 = fail-fast.")),
			mcp.WithString("reason", mcp.Description("Human-readable reason recorded in audit (e.g. 'udp perf sweep'). Strongly encouraged.")),
			mcp.WithString("agent_id", mcp.Description("Override the auto-derived agent identity. Normally leave blank; set only if you intend to share a lease across processes.")),
		),
	...
	addTool(s,
		mcp.NewTool("release",
			mcp.WithDescription("Release a lease this agent holds. Pass lease_id OR nodes (not both). Owner-only; use force_release to break someone else's lease."),
			mcp.WithString("lease_id", mcp.Description("The lease to release. Mutually exclusive with nodes.")),
			mcp.WithArray("nodes", mcp.Description("Release leases this agent holds that overlap with these nodes. Mutually exclusive with lease_id."), mcp.WithStringItems()),
			mcp.WithString("agent_id", mcp.Description("Override the auto-derived agent identity. Normally leave blank.")),
		),
	...
	addTool(s,
		mcp.NewTool("extend_lease",
			mcp.WithDescription("Bump the TTL on an active lease this agent owns. Useful when a test is running longer than planned."),
			mcp.WithString("lease_id", mcp.Required(), mcp.Description("Lease to extend.")),
			mcp.WithNumber("additional_minutes", mcp.Required(), mcp.Description("Minutes to add to the current expires_at.")),
			mcp.WithString("agent_id", mcp.Description("Override the auto-derived agent identity. Normally leave blank.")),
		),
	...
	addTool(s,
		mcp.NewTool("list_leases",
			mcp.WithDescription("Read-only view of currently held leases. Filter by agent, node, or include expired tombstones."),
			mcp.WithString("agent_id", mcp.Description("Only show leases owned by this agent id.")),
			mcp.WithString("node", mcp.Description("Only show leases covering this node.")),
			mcp.WithBoolean("include_expired", mcp.Description("If true, also show released/expired/forced rows. Default false.")),
	...
	addTool(s,
		mcp.NewTool("wait_for_release",
			mcp.WithDescription("Passively block until the given nodes have no active lease. Does NOT take a lease — call lease() afterwards to claim them. Returns freed=true on success or freed=false on timeout (NOT an error)."),
			mcp.WithArray("nodes", mcp.Required(), mcp.Description("Node names to watch."), mcp.WithStringItems()),
			mcp.WithNumber("timeout_seconds", mcp.Description("Maximum seconds to wait. Default 300.")),
		),
	...
	addTool(s,
		mcp.NewTool("lease_topology",
			mcp.WithDescription("Convenience: lease every node in a registered topology, atomically. Use this for fleet-wide tests."),
			mcp.WithString("topology", mcp.Required(), mcp.Description("Topology name (must already be registered via register_topology).")),
			mcp.WithNumber("duration_minutes", mcp.Description("Lease TTL. Default 60.")),
			mcp.WithNumber("wait_seconds", mcp.Description("If >0, block up to this many seconds waiting. Default 0 = fail-fast.")),
			mcp.WithString("reason", mcp.Description("Human-readable reason recorded in audit.")),
			mcp.WithString("agent_id", mcp.Description("Override auto-derived identity. Normally blank.")),
		),
	...
	addTool(s,
		mcp.NewTool("lease_role",
			mcp.WithDescription("Convenience: lease every node assigned to a given role, atomically. Pass topology= to scope to a specific topology, otherwise leases every node across all topologies with that role."),
			mcp.WithString("role", mcp.Required(), mcp.Description("Role name (e.g. 'server', 'client').")),
			mcp.WithString("topology", mcp.Description("Topology to scope to. If blank, every registered node with that role is included.")),
			mcp.WithNumber("duration_minutes", mcp.Description("Lease TTL. Default 60.")),
			mcp.WithNumber("wait_seconds", mcp.Description("If >0, block up to this many seconds waiting. Default 0 = fail-fast.")),
			mcp.WithString("reason", mcp.Description("Human-readable reason recorded in audit.")),
			mcp.WithString("agent_id", mcp.Description("Override auto-derived identity. Normally blank.")),
