...
func RegisterMultiNode(s *server.MCPServer, reg *registry.Registry, pool *agentclient.Pool, auditLog *audit.Log, leaseCfg LeaseGateConfig) {
	addTool(s,
		mcp.NewTool("run_script_on_role",
			mcp.WithDescription("Run an inline script on all nodes with a given role, in parallel."),
			mcp.WithString("role", mcp.Required(), mcp.Description("Role to target")),
			mcp.WithString("script_body", mcp.Required(), mcp.Description("Inline script content")),
			mcp.WithString("topology", mcp.Description("Topology to scope; omit for all")),
			mcp.WithString("shell", mcp.Description("Shell: powershell, bash")),
			mcp.WithNumber("timeout", mcp.Description("Timeout seconds per node")),
		),
	...
	addTool(s,
		mcp.NewTool("execute_on_role",
			mcp.WithDescription("Run a command on all nodes with a given role, in parallel."),
			mcp.WithString("role", mcp.Required(), mcp.Description("Role to target")),
