...
build-server:
	go build -o $(BIN_DIR)/LabLinkServer.exe ./cmd/server/

build-agent-windows:
	GOOS=windows GOARCH=amd64 go build -o $(BIN_DIR)/LabLinkAgent.exe ./cmd/agent/

build-agent-linux:
	GOOS=linux GOARCH=amd64 go build -o $(BIN_DIR)/LabLinkAgent-linux-amd64 ./cmd/agent/

build-ca:
	go build -o $(BIN_DIR)/lablink-ca.exe ./cmd/lablink-ca/

build-probe:
	go build -o $(BIN_DIR)/LabLinkProbe.exe ./cmd/probe/
